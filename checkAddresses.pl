...
MAIN: {
	...
	checkAddressInfo(%addressLine);
    }

    # Check that the src folder exists, and the final folder doesn't
    ...
    #tag the files
    tagFiles( $srcFolderName, $destFolderName, $photoMappingFile, @addresses);
...
sub readAddressFile {
    ...
    #print("Raw (unpacked) addresses are:\n");
    #$addressCount = scalar (@addresses);
    #for($i = 0 ; $i < $addressCount ; $i++) {
	#%addressLine = @{$addresses[$i]};
